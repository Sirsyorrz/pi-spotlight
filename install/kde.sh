...
kwriteconfig5 --file khotkeysrc --group "Data_${NEW_ID}" --key "Comment" "Spotlight Chat toggle"
kwriteconfig5 --file khotkeysrc --group "Data_${NEW_ID}" --key "Enabled" "true"
kwriteconfig5 --file khotkeysrc --group "Data_${NEW_ID}" --key "Name"    "Spotlight Chat"
...
echo "✓ KDE shortcut registered: Alt+Space → spotlight-chat"
echo "  If it conflicts, open System Settings → Shortcuts → Custom Shortcuts"
echo "  and reassign the key manually for 'Spotlight Chat'."
